<script lang="ts">
  ...
  import { setPairingsContext, type PairingsContext } from "$lib/model/Pairing";
  ...
  let ctx: PairingsContext = $state({ showOrganizerView: data.policy.update });

  setPairingsContext(ctx);
